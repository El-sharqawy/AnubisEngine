...
public:
	...
	virtual ~CDynamicPool()
	{
		...
		std::string traceback = std::format("[CDynamicPool<{}>]: Final used capacity : {}", typeid(T).name(), m_uiUsedCapacity);
		std::printf("%s", traceback.c_str());
#endif
	}
	...
	void Destroy()
	{
		...
		if (!m_vpData.empty())
		{
			std::string traceback = std::format("[CDynamicPool<{}>]: Destroying {} objects", typeid(T).name(), m_vpData.size());
			syslog("{}", traceback);
		}
#endif
	...
	T* Alloc()
	{
		if (m_vpFree.empty())
		{
			T* pNewData = AnubisNew(T, MEM_TAG_ENGINE);
			m_vpData.push_back(pNewData);
			m_uiUsedCapacity++;

#if defined(ENABLE_DYNAMIC_POOL_LOGS)
			syslog("[CDynamicPool<{}>] Allocated new object (total: {})", typeid(T).name(), m_uiUsedCapacity);
#endif
...
#if defined(ENABLE_DYNAMIC_POOL_LOGS)
		syslog("[CDynamicPool<{}>] Reused object from free list", typeid(T).name());
#endif
		return (pFreeData);
	}
	...
	void Free(T* pData)
	{
		...
		m_uiUsedCapacity--;
#if defined(ENABLE_DYNAMIC_POOL_LOGS)
		syslog("[CDynamicPool<{}>] Freed object", typeid(T).name());
#endif
	}
...
#if defined(ENABLE_DYNAMIC_POOL_LOGS)
		syslog("[CDynamicPool<{}>] All objects marked as free", typeid(T).name());
#endif
	}
